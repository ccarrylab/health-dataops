...
resource "aws_s3_bucket" "terraform_state" {
  ...
  tags   = { Name = "Terraform state"; Environment = var.environment }
}
...
resource "aws_dynamodb_table" "terraform_locks" {
  ...
  tags         = { Name = "Terraform locks"; Environment = var.environment }
}

output "state_bucket"  { value = aws_s3_bucket.terraform_state.id }
output "locks_table"   { value = aws_dynamodb_table.terraform_locks.name }
